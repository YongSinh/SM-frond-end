...
import { EditFilled, DeleteFilled, EyeFilled, RollbackOutlined } from "@ant-design/icons";
...
const PageStudentInClass = () => {
    ...
    const getList = () => {
        setLoading(true)
        request("get", "student/getStudentInClass/" + singleUserApiUrl, {}).then(res => {
            setLoading(false)
            if (res.status == 200) {
                var data = res.data
                setList(data.data)
...
export default PageStudentInClass;